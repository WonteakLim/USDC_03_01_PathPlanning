#include "poly.h"
#include <iostream>
#include "../utils/alglib/stdafx.h"
#include <math.h>
#include <stdlib.h>
#include <stdio.h>

#include "../utils/alglib/solvers.h"
...
double			poly_max_speed(const Eigen::VectorXd &coeffs, double t0, double t1)
{
	std::vector<double> time_points = {t0, t1};

	// find poles
	if( not_zero( coeffs[5] ) ){
	    std::vector<double> v_coef = { 2*coeffs[2], 6*coeffs[3], 12*coeffs[4], 20*coeffs[5] };
	    alglib::real_1d_array coeff;
	    coeff.setcontent( v_coef.size(), &(v_coef[0] ));
	    ...
	    for( int i=0; i<3; i++){
		double acc_zero = x[i].x;
		if( between( acc_zero, t0, t1 ) ){
		    time_points.push_back( acc_zero );
		}
	    }
	}
	else if( not_zero( coeffs[4]) ){
	    double a = 12*coeffs[4];
	    double b = 6*coeffs[3];
	    double c = 2*coeffs[2];
	    ...
	    if (d >= 0.0){
		    ...
		    double acc_zero1 = b - d;
		    if (between(acc_zero1, t0, t1)){
				time_points.push_back(acc_zero1);
		    }

		    double acc_zero2 = b + d;
		    if (between(acc_zero2, t0, t1)){
				time_points.push_back(acc_zero2);
		    }
	    }
	}
	else if( not_zero( coeffs[3]) ){
	    double acc_zero = -coeffs[2] / (3*coeffs[3] );
	    if( between( acc_zero, t0, t1 ) ){
		time_points.push_back( acc_zero );
	    }
	}
...
double			poly_min_acceleration(const Eigen::VectorXd &coeffs, double t0, double t1)
{
	vector<double> time_points = { t0, t1 };
	if (not_zero(coeffs[5])){
		double a = 60 * coeffs[5];
...
//    double max_acc = poly_max_acceleration( coeffs, t0, t1 );
//    if( max_acc < 0 ) max_acc = 0.0;
//    return max_acc;
//}
//
//double poly_min_acc_signed( const Eigen::VectorXd &coeffs, double t0, double t1){
//    double min_acc = poly_min_acceleration( coeffs, t0, t1 );
//    if( min_acc > 0 ) min_acc = 0.0;
//    return min_acc;
//}
